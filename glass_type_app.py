...
@st.cache()
def prediction(model, feature_cols):
	glass_type = model.predict([feature_cols])
	glass_type = glass_type[0]

	if glass_type == 1:
		return "building windows float processed".title()

	elif glass_type == 2:
		return "building windows non float processed".title()

	elif glass_type == 3:
		return "vehicle windows float processed".title()

	elif glass_type == 4:
		return "vehicle windows non float processed".title()

	elif glass_type == 5:
		return "containers".title()

	elif glass_type == 6:
		return "tableware".title()

	else:
		return "headlamp".title()

st.title('Glass Type Prediction WebApp')
...
st.sidebar.subheader('Visualisation Selecter')
...
if classifier == 'Support Vector Machine':
	...
	if st.sidebar.button('Classify'):
		...
		st.write('Accuracy is', accuracy.round(2))
		plot_confusion_matrix(svc_model, X_test, y_test)
		st.pyplot()

if classifier == 'Random Forest Classifier':
	st.sidebar.subheader('Model Hyperparamters')
	n_estimators_input = st.sidebar.number_input('Enter the number of trees', 100, 5000, step=10)
	max_depth_input = st.sidebar.number_input('Enter the maximum depth', 1, 100, step=1)
	if st.sidebar.button('Classify'):
